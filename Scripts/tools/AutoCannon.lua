...
function AutoCannon:client_onUpdate(dt)
...
function AutoCannon:client_onEquip(animate)
	if animate then
	...
	if self.isLocal then
		-- Sets AutoCannon renderable, change this to change the mesh
		self.tool:setFpRenderables(currentRenderablesFp)
		swapFpAnimation(self.fpAnimations, "unequip", "equip", 0.2)
	end
end

function AutoCannon:client_onUnequip(animate)
	self.wantEquipped = false
	self.equipped = false
	self.aiming = false
...
function AutoCannon:sv_n_onAim(aiming)
	self.network:sendToClients("cl_n_onAim", aiming)
end

function AutoCannon:cl_n_onAim(aiming)
	if not self.isLocal and self.tool:isEquipped() then
		self:onAim(aiming)
	end
end

function AutoCannon:onAim(aiming)
	self.aiming = aiming
...
function AutoCannon:calculateFirePosition()
...
function AutoCannon:calculateTpMuzzlePos()
	local crouching = self.tool:isCrouching()
	local dir = sm.localPlayer.getDirection()
	local pitch = math.asin(dir.z)
	local right = sm.localPlayer.getRight()
...
function AutoCannon:calculateFpMuzzlePos()
...
function AutoCannon:cl_onPrimaryUse(state)
	if state ~= 1 or self.fireCooldownTimer > 0 then
		return
	end
...
function AutoCannon:cl_onSecondaryUse(state)
...
function AutoCannon:client_onEquippedUpdate(lmb, rmb, f)
	if lmb ~= self.prevPrimaryState then
