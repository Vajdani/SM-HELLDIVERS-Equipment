...
function HMG:client_onUpdate(dt)
	...
	if self.isLocal then
		if self.equipped then
			...
			elseif not self.tool:isSprinting() and (currentAnim == "sprintIdle" or currentAnim == "sprintInto") then
				swapFpAnimation(self.fpAnimations, "sprintInto", "sprintExit", 0.0)
			end

			if self.aiming and not isAnyOf(currentAnim, { "aimInto", "aimIdle", "aimShoot" }) then
	...
	self.windupEffect:setPosition(pos)
...
function HMG:client_onEquip(animate)
	if animate then
	...
	if self.isLocal then
		-- Sets HMG renderable, change this to change the mesh
		self.tool:setFpRenderables(currentRenderablesFp)
		swapFpAnimation(self.fpAnimations, "unequip", "equip", 0.2)
	end
end

function HMG:client_onUnequip(animate)
...
function HMG:sv_n_onAim(aiming)
	self.network:sendToClients("cl_n_onAim", aiming)
end

function HMG:cl_n_onAim(aiming)
	if not self.isLocal and self.tool:isEquipped() then
		self:onAim(aiming)
	end
end

function HMG:onAim(aiming)
	self.aiming = aiming
...
function HMG:sv_n_onShoot()
	self.network:sendToClients("cl_n_onShoot")
end

function HMG:cl_n_onShoot()
	if not self.isLocal and self.tool:isEquipped() then
		self:onShoot()
	end
end

function HMG:onShoot()
...
function HMG:calculateFirePosition()
...
function HMG:calculateTpMuzzlePos()
	local crouching = self.tool:isCrouching()
	local dir = sm.localPlayer.getDirection()
	local pitch = math.asin(dir.z)
	local right = sm.localPlayer.getRight()
...
function HMG:calculateFpMuzzlePos()
...
function HMG:cl_updateGatling(dt)
...
function HMG:cl_onSecondaryUse(state)
...
function HMG:client_onEquippedUpdate(lmb, rmb, f)
	self.gatlingActive = lmb == 1 or lmb == 2
